...
pub type wasmtime_func_async_continuation_callback_t =
    extern "C" fn(*mut c_void, *mut wasmtime_caller_t, done: *mut bool) -> Option<Box<wasm_trap_t>>;
...
impl Future for CHostCallFuture<'_> {
    ...
    fn poll(self: Pin<&mut Self>, _cx: &mut Context) -> Poll<Self::Output> {
        ...
        let result = {
            ...
            cb(this.env.data, &mut this.caller, &mut done)
        };
...
pub extern "C" fn wasmtime_call_future_poll(future: &mut wasmtime_call_future_t) -> bool {
    ...
    let w = crate::fut::noop_waker_ref();
...
pub unsafe extern "C" fn wasmtime_func_call_async<'a>(
    ...
    let fut = Box::pin(func.call_async(store, &[], &mut []));
